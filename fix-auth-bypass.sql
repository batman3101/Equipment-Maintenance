...
-- 5. system_settings는 그대로 유지 (관리자만 접근)
-- 이미 적절한 정책이 있을 것으로 예상

-- 6. 권한 설정 수정 - anon 역할의 과도한 권한 제거
...
BEGIN
    ...
    -- profiles 접근 시도 (실패해야 함)
    BEGIN
        SELECT COUNT(*) INTO test_count FROM public.profiles;
        RAISE WARNING 'SECURITY ISSUE: anon can access profiles!';
    EXCEPTION
        WHEN OTHERS THEN
            RAISE NOTICE 'Good: anon cannot access profiles';
    END;
